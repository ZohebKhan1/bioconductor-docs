...
In order to better demonstrate the usage of `plotScoreLandscape`, we load some additional datasets consisting of pre-computed scores of larger public datasets. `scoredf_ccle_epi` and `scoredf_ccle_mes` are two scored results of a CCLE dataset [(Barretina et al. [2012](#ref-barretina2012cancer){role="doc-biblioref"})] against an epithelial gene signature and mesenchymal gene signature [(Tan et al. [2014](#ref-Ep-MES){role="doc-biblioref"})] respectively. For details on how to obtain the dataset please see the section at the end of the vignette.
...
Similarly, pre-computed scores for the [TCGA](https://cancergenome.nih.gov/) breast cancer RNA-seq dataset against epithelial and mesenchymal gene signatures are stored in `scoredf_tcga_epi` and `scoredf_tcga_mes` respectively [(Tan et al. [2014](#ref-Ep-MES){role="doc-biblioref"})]. The utility of this function is enhanced when the number of samples is large.
...
# [4] Estimate empirical p-values for the obtained scores in individual samples and plot null distributions
...
## [4.2] Estimate empirical p-values
...
`scoredf_ccle_epi` and `scoredf_ccle_mes` are two data frames of the pre-computed scores using The Cancer Cell Line Encyclopaedia (CCLE) breast cancer cell line RNA-seq dataset [(Barretina et al. [2012](#ref-barretina2012cancer){role="doc-biblioref"})]. The CCLE dataset was normalised by TPM and can be downloaded from <https://www.synapse.org/#!Synapse:syn5612998> Cell lines were scored against the epithelial and mesenchymal gene signatures, which were obtained from [(Tan et al. [2014](#ref-Ep-MES){role="doc-biblioref"})] and can be found in the 'Table S1B. Generic EMT signature for cell line' in the supplementary files.

3.  TCGA cancer samples dataset

    `scoredf_tcga_epi` and `scoredf_tcga_mes` are two data frames of the pre-computed scores using The Cancer Genome Atlas (TCGA) breast cancer RNA-seq data (RSEM normalised) [(The Cancer Genome Atlas Network [2012](#ref-TCGAcite){role="doc-biblioref"})] against the epithelial and mesenchymal gene signatures respectively. The gene signatures were obtained from [(Tan et al. [2014](#ref-Ep-MES){role="doc-biblioref"})] and can be found in the 'Table S1A. Generic EMT signature for tumour'. The TCGA dataset was downloaded from The UCSC Cancer Genomics Browser in February 2016 (<https://genome-cancer.ucsc.edu>)
...
```
    ...
    ##  [1] GSEABase_1.72.0      graph_1.88.0         annotate_1.88.0
    ##  [4] XML_3.99-0.19        AnnotationDbi_1.72.0 IRanges_2.44.0
    ##  [7] S4Vectors_0.48.0     Biobase_2.70.0       BiocGenerics_0.56.0
    ## [10] generics_0.1.4       singscore_1.30.0     BiocStyle_2.38.0
    ##
    ...
    ## [27] BiocParallel_1.44.0         abind_1.4-8
    ## [29] withr_3.0.2                 purrr_1.1.0
    ## [31] grid_4.5.1                  xtable_1.8-4
    ## [33] edgeR_4.8.0                 ggplot2_4.0.0
    ## [35] scales_1.4.0                dichromat_2.0-0.1
    ## [37] tinytex_0.57                SummarizedExperiment_1.40.0
    ## [39] cli_3.6.5                   rmarkdown_2.30
    ## [41] crayon_1.5.3                httr_1.4.7
    ## [43] reshape2_1.4.4              DBI_1.2.3
    ...
    ## [55] ggrepel_0.9.6               bit64_4.6.0-1
    ## [57] magick_2.9.0                locfit_1.5-9.12
    ## [59] limma_3.66.0                hexbin_1.28.5
...
Foroutan, Momeneh, Dharmesh D Bhuva, Ruqian Lyu, Kristy Horan, Joseph Cursons, and Melissa J Davis. 2018. "Single Sample Scoring of Molecular Phenotypes." *BMC Bioinformatics* 19 (1): 404. <https://doi.org/10.1186/s12859-018-2435-4>.
