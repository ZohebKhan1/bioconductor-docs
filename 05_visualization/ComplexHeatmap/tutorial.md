...
The style of legends can be controlled by `heatmap_legend_param` in `Heatmap()`, or `annotation_legend_param` in `HeatmapAnnotation()`. The parameters for controlling legends are those arguments in `Legend()` function. See <https://jokergoo.github.io/ComplexHeatmap-reference/book/legends.html#heatmap-and-annotation-legends>.
